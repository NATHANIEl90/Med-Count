...
if (isset($_POST['id'])) {
    $id = $_POST['id'];
    // Update the record's status to 'Deleted' instead of deleting it
    $update_sql = "UPDATE supply_requests SET status = 'Deleted' WHERE id = ?";

    // Prepare statement
    if ($stmt = mysqli_prepare($conn, $update_sql)) {
        // Bind the integer parameter for the marker
        mysqli_stmt_bind_param($stmt, "i", $id);  // Use 'i' for integer id
